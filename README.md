...
Supplies SSH credentials to `keeper.push` from a static key on disk or in Vault.

A Soul Stack **SshProvider** plugin, built as `soul-ssh-static` and launched by the host over
...
make build      # dist/soul-ssh-static
```
...
It lived in `soul-stack/examples/module/soul-ssh-static` until NIM-825. [ADR-011][adr11]
